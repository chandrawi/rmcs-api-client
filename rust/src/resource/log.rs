...
pub(crate) async fn create_log(resource: &Resource, timestamp: DateTime<Utc>, device_id: Option<Uuid>, model_id: Option<Uuid>, tag: i16, value: DataValue)
    -> Result<i32, Status>
{
    let interceptor = TokenInterceptor(resource.access_token.clone());
    let mut client = 
...
pub(crate) async fn update_log(resource: &Resource, id: i32, tag: Option<i16>, value: Option<DataValue>)
    -> Result<(), Status>
{
    let interceptor = TokenInterceptor(resource.access_token.clone());
    let mut client = 
